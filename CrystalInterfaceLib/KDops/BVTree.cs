...
namespace CrystalInterfaceLib.KDops
{
	...
	public class BVTree
	{
			...
			ref AtomInfo[] leftAtoms, ref AtomInfo[] rightAtoms)
		{
			...
			List<AtomInfo> leftAtomList = new List<AtomInfo> ();
			List<AtomInfo> rightAtomList = new List<AtomInfo> ();
		...
		{
			Dictionary<string, List<AtomInfo>> residueSeqListHash = new Dictionary<string,List<AtomInfo>> ();
			foreach (AtomInfo atom in atoms)
			{
                if (residueSeqListHash.ContainsKey(atom.seqId))
				{
                    residueSeqListHash[atom.seqId].Add(atom);
				}
				else
				{
					List<AtomInfo> atomList = new List<AtomInfo> ();
					atomList.Add (atom);
                    residueSeqListHash.Add(atom.seqId, atomList);
				}
			}
            Residue[] residues = new Residue[residueSeqListHash.Count];
			int count = 0;
            foreach (string seqId in residueSeqListHash.Keys)
			{
                AtomInfo[] residueAtoms = residueSeqListHash[seqId].ToArray (); 
		...
		{
			Dictionary<string, Residue> residueSeqHash = new Dictionary<string,Residue> ();
